...
$bugster_featured = in_array($bugster_post_format, array("image", "video", "gallery")) || has_post_thumbnail() ? true : false;
?>

<article id="post-<?php the_ID(); ?>" 
	<?php post_class( 'post_item post_layout_modern post_format_' . esc_attr( $bugster_post_format ) ); ?>
...
>
    <?php
    if($bugster_featured) { ?>
        <div class="post_container_featured"><?php
    }
	// Featured image
	...
	bugster_show_post_featured(
		array(
			...
			'hover'      => $bugster_hover,
			'thumb_size' => bugster_get_thumb_size( strpos( bugster_get_theme_option( 'body_style' ), 'full' ) !== false ? 'full' : ( $bugster_expanded ? 'modern' : 'modern' ) ),
		)
    );
    if($bugster_featured) { ?>
        </div><?php
    ...
    if($bugster_featured) { ?>
        <div class="post_container_content"><?php
    }
