			...
			if ( $need_content )  {
				...
				?><div class="sc_layouts_title_content"><?php
					...
					if (!empty($args['meta']) && is_single() )  {
						...
						?><div class="sc_layouts_title_meta sc_layouts_title_meta-top"><?php
							...
							$bugster_post_meta1 = (in_array('categories',$bugster_components_arr) ? 'categories,' : '');
							if ( ! empty( $bugster_components ) ) {
								bugster_show_post_meta(
									apply_filters(
										'bugster_filter_post_meta_args', array(
											'components' => $bugster_post_meta1,
											'seo'        => false,
										), 'excerpt', 1
									)
								);
						...
						?><div class="sc_layouts_title_title"><?php
							...
							<h1<?php trx_addons_seo_snippets('headline'); ?> class="sc_layouts_title_caption<?php echo esc_attr($trx_addons_blog_title_class); ?>"><?php
								...
								if (!empty($trx_addons_top_icon)) {
									...
									?><img src="<?php echo esc_url($trx_addons_top_icon); ?>" alt="<?php esc_attr_e('Icon', 'bugster'); ?>" <?php if (!empty($trx_addons_attr[3])) trx_addons_show_layout($trx_addons_attr[3]);?>><?php
								}
					...
					if (!empty($args['meta']) && is_single() )  {
						...
						?><div class="sc_layouts_title_meta sc_layouts_title_meta-bottom"><?php

							$bugster_components = bugster_array_get_keys_by_value( bugster_get_theme_option( 'meta_parts' ) );
							$bugster_components_arr = explode(',', $bugster_components);
							$bugster_post_meta2 = (in_array('date',$bugster_components_arr) ? 'date,' : '')
													. (in_array('author',$bugster_components_arr) ? 'author,' : '')
													. (in_array('views',$bugster_components_arr) ? 'views,' : '')
													. (in_array('comments',$bugster_components_arr) ? 'comments,' : '')
													. (in_array('likes',$bugster_components_arr) ? 'likes,' : '');

							if ( ! empty( $bugster_components ) ) {
								bugster_show_post_meta(
									apply_filters(
										'bugster_filter_post_meta_args', array(
											'components' => $bugster_post_meta2,
											'seo'        => false,
										), 'excerpt', 1
									)
								);
