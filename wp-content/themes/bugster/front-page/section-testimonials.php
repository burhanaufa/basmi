<div class="front_page_section front_page_section_testimonials<?php
	$bugster_scheme = bugster_get_theme_option( 'front_page_testimonials_scheme' );
	if ( ! bugster_is_inherit( $bugster_scheme ) ) {
		echo ' scheme_' . esc_attr( $bugster_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( bugster_get_theme_option( 'front_page_testimonials_paddings' ) );
?>"
		<?php
		$bugster_css      = '';
		$bugster_bg_image = bugster_get_theme_option( 'front_page_testimonials_bg_image' );
...
<?php
	// Add anchor
	$bugster_anchor_icon = bugster_get_theme_option( 'front_page_testimonials_anchor_icon' );
	$bugster_anchor_text = bugster_get_theme_option( 'front_page_testimonials_anchor_text' );
...
?>
	...
	<?php
	if ( bugster_get_theme_option( 'front_page_testimonials_fullheight' ) ) {
	...
	?>
			...
			<?php
			$bugster_css      = '';
			$bugster_bg_mask  = bugster_get_theme_option( 'front_page_testimonials_bg_mask' );
			$bugster_bg_color_type = bugster_get_theme_option( 'front_page_testimonials_bg_color_type' );
			if ( 'custom' == $bugster_bg_color_type ) {
				$bugster_bg_color = bugster_get_theme_option( 'front_page_testimonials_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_testimonials_content_wrap content_wrap">
			<?php
			// Caption
			$bugster_caption = bugster_get_theme_option( 'front_page_testimonials_caption' );
			if ( ! empty( $bugster_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_testimonials_caption front_page_block_<?php echo ! empty( $bugster_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses_post( $bugster_caption ); ?></h2>
				<?php
			}

			// Description (text)
			$bugster_description = bugster_get_theme_option( 'front_page_testimonials_description' );
			if ( ! empty( $bugster_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_testimonials_description front_page_block_<?php echo ! empty( $bugster_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses_post( wpautop( $bugster_description ) ); ?></div>
				<?php
			}
			...
			if ( is_active_sidebar( 'front_page_testimonials_widgets' ) ) {
				...
				if ( ! bugster_exists_trx_addons() ) {
					...
					bugster_customizer_need_widgets_message( 'front_page_testimonials_caption', 'ThemeREX Addons - Testimonials' );
				}
			}
			?>
			</div>
