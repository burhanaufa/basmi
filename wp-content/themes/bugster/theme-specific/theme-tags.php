...
if ( ! function_exists( 'bugster_need_page_title' ) ) {
	function bugster_need_page_title() {
		return ! is_front_page() && apply_filters( 'bugster_filter_need_page_title', true );
	}
}
...
if ( ! function_exists( 'bugster_show_layout' ) ) {
	function bugster_show_layout( $str, $before = '', $after = '' ) {
		if ( trim( $str ) != '' ) {
...
if ( ! function_exists( 'bugster_get_logo_image' ) ) {
	function bugster_get_logo_image( $type = '' ) {
		$logo_image  = '';
		if ( empty( $type ) && function_exists( 'the_custom_logo' ) ) {
			$logo_image = bugster_get_theme_option( 'custom_logo' );
			if ( empty( $logo_image ) ) {
			...
			$logo_image = bugster_get_theme_option( 'logo' . ( ! empty( $type ) ? '_' . trim( $type ) : '' ) );
		}
		$logo_retina = bugster_is_on( bugster_get_theme_option( 'logo_retina_enabled' ) )
						? bugster_get_theme_option( 'logo' . ( ! empty( $type ) ? '_' . trim( $type ) : '' ) . '_retina' )
						: '';
		return array(
					'logo'        => ! empty( $logo_image ) ? bugster_remove_protocol_from_url( $logo_image, false ) : '',
					'logo_retina' => ! empty( $logo_retina ) ? bugster_remove_protocol_from_url( $logo_retina, false ) : ''
				);
	}
}

// Return header video (if set)
if ( ! function_exists( 'bugster_get_header_video' ) ) {
	function bugster_get_header_video() {
		$video = '';
		if ( apply_filters( 'bugster_header_video_enable', ! wp_is_mobile() && is_front_page() ) ) {
			if ( bugster_check_theme_option( 'header_video' ) ) {
				$video = bugster_get_theme_option( 'header_video' );
				if ( (int) $video > 0 ) {
...
// Show post banner
if ( ! function_exists( 'bugster_show_post_banner' ) ) {
	function bugster_show_post_banner( $banner_pos = '') {
		if ( is_singular( 'post' ) && '' !== $banner_pos ){
			$banner_code   = bugster_get_theme_option( $banner_pos . '_banner_code' );
			$banner_img    = bugster_get_theme_option( $banner_pos . '_banner_img' );
			$banner_height = bugster_get_theme_option( $banner_pos . '_banner_height' );
			$banner_class  = !empty( $banner_img )
								? ( 'background' == $banner_pos ? '' : 'banner_with_image ' )
									. bugster_add_inline_css_class( 
										'background-image:url(' . esc_url( $banner_img ) . ');'
										. ( ! empty( $banner_height )
											? 'min-height:' . bugster_prepare_css_value( $banner_height ) . ';'
											: ''
											)
										)
								: '';
			$banner_link   = bugster_get_theme_option( $banner_pos . '_banner_link' );
			if ( ! empty( $banner_code ) || ! empty( $banner_img ) ) {
				...
				bugster_show_layout(wp_kses_post ($banner_code));
...
if ( ! function_exists( 'bugster_show_post_featured_image' ) ) {
	function bugster_show_post_featured_image() {
		$seo = bugster_is_on( bugster_get_theme_option( 'seo_snippets' ) );
		// Featured image
		if ( bugster_is_off( bugster_get_theme_option( 'hide_featured_on_single' ) )
					&& ! bugster_sc_layouts_showed( 'featured' )
					&& strpos( bugster_get_post_content(), '[trx_widget_banner]' ) === false ) {
			do_action( 'bugster_action_before_post_featured' );
			bugster_show_post_featured(
				array(
					'singular' => true,
					'thumb_bg' => bugster_get_theme_option( 'post_thumbnail_type' ) == 'fullwidth'
				)
			);
...
if ( ! function_exists( 'bugster_show_post_title_and_meta' ) ) {
	function bugster_show_post_title_and_meta() {

		$seo = bugster_is_on( bugster_get_theme_option( 'seo_snippets' ) );

		// Title and post meta
		if ( ( ! bugster_sc_layouts_showed( 'title' ) || ! bugster_sc_layouts_showed( 'postmeta' ) ) && ! in_array( get_post_format(), array( 'link', 'aside', 'status', 'quote' ) ) ) {
			do_action( 'bugster_action_before_post_title' );
			$need_content_wrap = bugster_get_theme_option( 'post_thumbnail_type' ) == 'fullwidth' && bugster_get_theme_option( 'post_header_position' ) !== 'on_thumb';
			?>
			<div class="post_header post_header_single entry-header">
				...
				// Post meta
				$meta_components = bugster_array_get_keys_by_value( bugster_get_theme_option( 'meta_parts' ) );
				...
				if ( $show_categories && ! bugster_sc_layouts_showed( 'postmeta' ) && bugster_is_on( bugster_get_theme_option( 'show_post_meta' ) ) ) {
					bugster_show_post_meta(
						apply_filters(
							'bugster_filter_post_meta_args',
							array(
				...
				// Post title
				if ( ! bugster_sc_layouts_showed( 'title' ) ) {
					the_title( '<h1 class="post_title entry-title"' . ( $seo ? ' itemprop="headline"' : '' ) . '>', '</h1>' );
				}
				// Post subtitle
				$post_subtitle = bugster_get_theme_option( 'post_subtitle' );
				if ( ! empty( $post_subtitle ) ) {
					?>
					<div class="post_subtitle">
						<?php bugster_show_layout( $post_subtitle ); ?>
					</div>
					<?php
				}
				// Post meta
				if ( ! bugster_sc_layouts_showed( 'postmeta' ) && bugster_is_on( bugster_get_theme_option( 'show_post_meta' ) ) ) {
					bugster_show_post_meta(
						apply_filters(
							'bugster_filter_post_meta_args',
							array(
			...
			<?php
			do_action( 'bugster_action_after_post_title' );
		}
	}
}
...
if ( ! function_exists( 'bugster_show_post_content' ) ) {
	function bugster_show_post_content( $args = array(), $otag='', $ctag='' ) {
		$plain = true;
		...
		if ( has_excerpt() ) {
			...
			bugster_show_layout( bugster_filter_post_content( get_the_content('') ) );
			...
			$quote = bugster_get_tag( bugster_filter_post_content( get_the_content() ), '<blockquote', '</blockquote>' );
			if ( ! empty( $quote ) ) {
				bugster_show_layout( wpautop( $quote ) );
				$plain = false;
			} else {
				bugster_show_layout( bugster_filter_post_content( get_the_content() ) );
			}
			...
			if ( $plain ) {
				$len = ! empty( $args['hide_excerpt'] )
							...
							: ( ! empty( $args['excerpt_length'] )
								...
								: ($blog_style[0] === 'modern' && (in_array($post_format, array("image", "video", "gallery")) || has_post_thumbnail()) ? bugster_get_theme_option( 'excerpt_modern_length' ) : bugster_get_theme_option( 'excerpt_length' ))
								);
				$output = bugster_excerpt( $output, $len );
			}
		}
		bugster_show_layout( $output, $otag, $ctag);
	}
}


// Show post link 'Read more' in the blog posts
if ( ! function_exists( 'bugster_show_post_more_link' ) ) {
	function bugster_show_post_more_link( $args = array(), $otag='', $ctag='' ) {
		bugster_show_layout(
			'<a class="more-link" href="' . esc_url( get_permalink() ) . '">'
				. ( ! empty( $args['more_text'] )
						? esc_html( $args['more_text'] )
						: esc_html__( 'Read more', 'bugster' )
						)
			. '</a>',
			$otag,
			$ctag
...
if ( ! function_exists( 'bugster_show_post_meta' ) ) {
	function bugster_show_post_meta( $args = array() ) {
		if ( is_single() && bugster_is_off( bugster_get_theme_option( 'show_post_meta' ) ) ) {
		...
		<div class="post_meta<?php echo ! empty( $args['class'] ) ? ' ' . esc_attr( $args['class'] ) : ''; ?>">
			...
			foreach ( $components as $comp ) {
				...
				if ( 'categories' == $comp ) {
					// Post categories
					$cats = get_post_type() == 'post' ? get_the_category_list( ' ' ) : apply_filters( 'bugster_filter_get_post_categories', '' );
					if ( ! empty( $cats ) ) {
						bugster_show_layout( $cats, '<span class="post_meta_item post_categories">', '</span> ');
					}
				} elseif ( 'date' == $comp ) {
					// Post date
					$dt = apply_filters( 'bugster_filter_get_post_date', bugster_get_date( '', ! empty( $args['date_format'] ) ? $args['date_format'] : '' ) );
					if ( ! empty( $dt ) ) {
						bugster_show_layout(
							$dt,
					...
					if ( $author_id > 0 ) {
						...
						$author_avatar = get_avatar( get_the_author_meta( 'user_email', $author_id ), 32 * bugster_get_retina_multiplier() );
						echo '<a class="post_meta_item post_author" rel="author" href="' . esc_url( $author_link ) . '">'
								. ( bugster_get_theme_setting( 'show_author_avatar' )
					...
					if ( !is_singular() || have_comments() || comments_open() ) {
						...
						echo '<a href="' . esc_url( get_comments_link() ) . '" class="post_meta_item post_meta_comments icon-comment-light">'
								...
								. '<span class="post_meta_label">' . esc_html( _n( 'Comment', 'Comments', $post_comments, 'bugster' ) ) . '</span>'
							. '</a> ';
					}
					...
					if ( function_exists( 'trx_addons_get_post_views' ) ) {
						...
						echo ( is_singular()
							...
							. ' class="post_meta_item post_meta_views trx_addons_icon-eye">'
								...
								. '<span class="post_meta_label">' . esc_html( _n( 'View', 'Views', $post_views, 'bugster' ) ) . '</span>'
							. ( is_singular()
								? '</span> '
								: '</a> '
								);
				...
				// Likes (Emotions)
					...
					if ( function_exists( 'trx_addons_get_post_likes' ) ) {
						...
						echo ( true == $emotions_allowed
								...
								: '<a href="#"'
									...
									. ' title="' . ( ! empty( $allow ) ? esc_attr__( 'Like', 'bugster') : esc_attr__( 'Dislike', 'bugster' ) ) . '"'
									. ' data-postid="' . esc_attr( get_the_ID() ) . '"'
									. ' data-likes="' . esc_attr( $post_likes ) . '"'
									. ' data-title-like="' . esc_attr__( 'Like', 'bugster') . '"'
									. ' data-title-dislike="' . esc_attr__( 'Dislike', 'bugster' ) . '"'
									. '>'
								)
									. '<span class="post_meta_number">' . esc_html($post_likes) . '</span>'
									. '<span class="post_meta_label">'
										. ( true == $emotions_allowed
											? esc_html( _n( 'Reaction', 'Reactions', $post_likes, 'bugster' ) )
											: esc_html( _n( 'Like', 'Likes', $post_likes, 'bugster' ) )
											)
									. '</span>'
								. '</a> ';
					}
					...
					bugster_show_share_links(
						array(
							...
							'caption' => 'drop' == $args['share_type'] ? esc_html__( 'Share', 'bugster' ) : '',
					...
					// Edit page link
					edit_post_link( esc_html__( 'Edit', 'bugster' ), '', '', 0, 'post_meta_item post_edit icon-pencil' );

				} else {
					// Custom counter
					do_action( 'bugster_action_show_post_meta', $comp, get_the_ID(), $args );
				}
			}
			?>
		</div><!-- .post_meta -->
...
if ( ! function_exists( 'bugster_show_post_featured' ) ) {
	function bugster_show_post_featured( $args = array() ) {

		$args = array_merge(
			array(
				'hover'         => bugster_get_theme_option( 'image_hover' ), // Hover effect
		...
		$thumb_size  = ! empty( $args['thumb_size'] )
						? $args['thumb_size']
						: bugster_get_thumb_size( is_attachment() || is_single() ? 'full' : 'big' );
		$post_format = str_replace( 'post-format-', '', get_post_format() );
		$no_image    = ! empty( $args['show_no_image'] ) ? bugster_get_no_image( '', true ) : '';
		if ( $args['thumb_bg'] ) {
			if ( has_post_thumbnail() ) {
				$image = wp_get_attachment_image_src( get_post_thumbnail_id( get_the_ID() ), $thumb_size );
				$image = $image[0];
			} elseif ( 'image' == $post_format ) {
				$image = bugster_get_post_image();
				if ( ! empty( $image ) ) {
					$image = bugster_add_thumb_size( $image, $thumb_size );
				}
			}
			if ( empty( $image ) ) {
				$image = $no_image;
			...
			if ( ! empty( $image ) ) {
				$args['class'] .= ( $args['class'] ? ' ' : '' ) . 'post_featured_bg' . ' ' . bugster_add_inline_css_class( 'background-image: url(' . esc_url( $image ) . ');' );
			}
		}
		...
		if ( ! empty( $args['singular'] ) ) {
			...
			if ( is_attachment() ) {
				...
				if ( ! $args['thumb_bg'] ) {
					echo wp_get_attachment_image(
						...
						bugster_is_on( bugster_get_theme_option( 'seo_snippets' ) )
													? array( 'itemprop' => 'image' )
						: ''
					);
				}
				if ( bugster_get_theme_setting( 'attachments_navigation' ) ) {
					?>
				...
				$output .= '>';
				bugster_show_layout( $output );
				if ( has_post_thumbnail() && $args['seo'] ) {
					$bugster_attr = bugster_getimagesize( wp_get_attachment_url( get_post_thumbnail_id() ) );
					?>
						<meta itemprop="width" content="<?php echo esc_attr( $bugster_attr[0] ); ?>">
						<meta itemprop="height" content="<?php echo esc_attr( $bugster_attr[1] ); ?>">
						<?php
				}
				if ( ! $args['thumb_bg'] ) {
					if ( has_post_thumbnail() ) {
			...
			if ( $has_thumb
				...
				if ( $args['thumb_bg'] ) {
					...
					$output = bugster_get_slider_layout( $slider_args );
					if ( '' != $output ) {
						bugster_show_layout( $output );
					}
					//------- End: Moved down --------
				...
				if ( ! $args['thumb_only'] && ( in_array( $post_format, array( 'video', 'audio', 'gallery' ) ) ) ) {
					$post_content = bugster_get_post_content();
					$post_content_parsed = $post_content;

					if ( 'video' == $post_format ) {
						$video = bugster_get_post_video( $post_content, false );
						if ( empty( $video ) ) {
							$video = bugster_get_post_iframe( $post_content, false );
						}
						if ( empty( $video ) ) {
							// Only get video from the content if a playlist isn't present.
							$post_content_parsed = bugster_filter_post_content( $post_content );
							if ( false === strpos( $post_content_parsed, 'wp-playlist-script' ) ) {
								...
								if ( ! empty( $videos ) && is_array( $videos ) ) {
									$video = bugster_array_get_first( $videos, false );
								}
							}
						}
						if ( ! empty( $video ) ) {
							if ( $has_thumb ) {
								$video = bugster_make_video_autoplay( $video );
								?>
						...
						$audio = bugster_get_post_audio( $post_content, false );
						if ( empty( $audio ) ) {
							$audio = bugster_get_post_iframe( $post_content, false );
						}
						...
						if ( empty( $audio ) ) {
							...
							if ( false === strpos( $post_content_parsed, 'wp-playlist-script' ) ) {
								...
								if ( ! empty( $audios ) && is_array( $audios ) ) {
									$audio = bugster_array_get_first( $audios, false );
								}
							}
						}
						if ( ! empty( $audio ) ) {
							...
							">
								...
								if ( strpos( $audio, '<audio' ) !== false ) {
									$media_author = bugster_get_tag_attrib( $audio, '<audio>', 'data-author' );
									$media_title  = bugster_get_tag_attrib( $audio, '<audio>', 'data-caption' );
								}
								if ( empty( $media_author) &&  empty( $media_title) ) {
									$media = urldecode( bugster_get_tag_attrib( $post_content, '[trx_widget_audio]', 'media' ) );
									if ( ! empty( $media ) ) {
										...
										// Parse tag params
										$media_author = strip_tags( bugster_get_tag( $post_content_parsed, '<h6 class="audio_author">', '</h6>' ) );
										$media_title  = strip_tags( bugster_get_tag( $post_content_parsed, '<h5 class="audio_caption">', '</h5>' ) );

									}
								}
								if ( ! empty( $media_author ) ) {
									?>
									<div class="post_audio_author"><?php bugster_show_layout( $media_author ); ?></div>
									<?php
								}
								if ( ! empty( $media_title ) ) {
									?>
									<h5 class="post_audio_title"><?php bugster_show_layout( $media_title ); ?></h5>
									<?php
								}
								// Display audio
								bugster_show_layout( $audio );
								?>
							</div>
							<?php
						}
...
if ( ! function_exists( 'bugster_get_no_image' ) ) {
	function bugster_get_no_image( $no_image = '', $need = false ) {
		static $no_image_url = '';
		$img = bugster_get_theme_option( 'no_image' );
		if ( empty( $img ) && ( $need || bugster_get_theme_setting( 'allow_no_image' ) ) ) {
			if ( empty( $no_image_url ) ) {
				$no_image_url = bugster_get_file_url( 'images/no-image.jpg' );
			}
			$img = $no_image_url;
		}
		if ( ! empty( $img ) ) {
...
if ( ! function_exists( 'bugster_add_bg_in_post_nav' ) ) {
	function bugster_add_bg_in_post_nav() {
		if ( ! is_single() ) {
			return;
		}
		...
		if ( $previous ) {
			...
			if ( has_post_thumbnail( $previous->ID ) ) {
				$img = wp_get_attachment_image_src( get_post_thumbnail_id( $previous->ID ), bugster_get_thumb_size( 'med' ) );
				$img = $img[0];
			} else {
				$img = $noimg;
			}
		...
		if ( $next ) {
			...
			if ( has_post_thumbnail( $next->ID ) ) {
				$img = wp_get_attachment_image_src( get_post_thumbnail_id( $next->ID ), bugster_get_thumb_size( 'med' ) );
				$img = $img[0];
			} else {
				$img = $noimg;
			}
		...
		wp_add_inline_style( 'bugster-main', $css );
	}
}

// Show related posts
if ( ! function_exists( 'bugster_show_related_posts' ) ) {
	function bugster_show_related_posts( $args = array(), $style = 1, $title = '' ) {
		$args = array_merge(
			array(
		...
		$taxonomy = 'post' == $args['post_type'] ? 'category' : bugster_get_post_type_taxonomy();
		...
		if ( $query->found_posts > 0 ) {
			...
			$args['slider'] = (int) bugster_get_theme_option( 'related_slider' ) && min( $args['posts_per_page'], $query->found_posts) > $columns;
			$related_position = bugster_get_theme_option( 'related_position' );
			$related_style = bugster_get_theme_option( 'related_style' );
			...
			<section class="related_wrap related_position_<?php echo esc_attr( $related_position ); ?> related_style_<?php echo esc_attr( $related_style ); ?>">
				<<?php echo esc_html( $related_tag ); ?> class="section_title related_wrap_title"><?php
					if ( ! empty( $title ) ) {
						...
						esc_html_e( 'Related Posts', 'bugster' );
					}
				...
				if ( $args['slider'] ) {
					...
					$slider_args['slides_space']      = bugster_get_theme_option( 'related_slider_space' );
					$slider_args['slider_controls']   = bugster_get_theme_option( 'related_slider_controls' );
					$slider_args['slider_pagination'] = bugster_get_theme_option( 'related_slider_pagination' );
					$slider_args                      = apply_filters( 'bugster_related_posts_slider_args', $slider_args, $query );
					?><div class="related_wrap_slider"><?php
					bugster_get_slider_wrap_start('related_posts_wrap', $slider_args);
				} else {
					...
					while ( $query->have_posts() ) {
						...
						get_template_part( apply_filters( 'bugster_filter_get_template_part', 'templates/related-posts', $style ), $style );
						?></div><?php
					}
				?></div><?php
				if ( $args['slider'] ) {
					bugster_get_slider_wrap_end('related_posts_wrap', $slider_args);
...
if ( ! function_exists( 'bugster_show_related_posts_callback' ) ) {
	add_action( 'bugster_action_related_posts', 'bugster_show_related_posts_callback' );
	function bugster_show_related_posts_callback() {
		if ( is_single() && ! apply_filters( 'bugster_filter_show_related_posts', false ) ) {
			$bugster_related_posts   = (int) bugster_get_theme_option( 'related_posts' );
			$bugster_related_columns = (int) bugster_get_theme_option( 'related_columns' );
			$bugster_related_style   = bugster_get_theme_option( 'related_style' );
			if ( (int) bugster_get_theme_option( 'show_related_posts' ) && $bugster_related_posts > 0 ) {
				bugster_show_related_posts(
					array(
						'orderby'        => 'rand',
						'posts_per_page' => max( 1, min( 9, $bugster_related_posts ) ),
						'columns'        => max( 1, min( 6, $bugster_related_posts, $bugster_related_columns ) ),
					),
					$bugster_related_style
				);
			}
		}
	}
...
if ( ! function_exists( 'bugster_show_portfolio_posts' ) ) {
	function bugster_show_portfolio_posts( $args = array() ) {
		$args = array_merge(
			array(
				'cat'        => 0,
				'parent_cat' => 0,
		...
		$blog_style = explode( '_', empty( $args['blog_style'] ) ? bugster_get_theme_option( 'blog_style' ) : $args['blog_style'] );
		$style      = $blog_style[0];
		...
		if ( ! $args['echo'] ) {
			...
			$q_args = bugster_query_add_posts_and_cats( $q_args, '', $args['post_type'], $args['cat'], $args['taxonomy'] );
			if ( $args['page'] > 1 ) {
				$q_args['paged']               = $args['page'];
				$q_args['ignore_sticky_posts'] = true;
			}
			$ppp = bugster_get_theme_option( 'posts_per_page' );
			if ( 0 != (int) $ppp ) {
		...
		while ( have_posts() ) {
			...
			$bugster_part = $args['sticky'] && is_sticky() ? 'sticky' : ( 'gallery' == $style ? 'portfolio-gallery' : $style );
...
// AJAX handler for the bugster_ajax_get_posts action
if ( ! function_exists( 'bugster_ajax_get_posts_callback' ) ) {
	add_action( 'wp_ajax_bugster_ajax_get_posts', 'bugster_ajax_get_posts_callback' );
	add_action( 'wp_ajax_nopriv_bugster_ajax_get_posts', 'bugster_ajax_get_posts_callback' );
	function bugster_ajax_get_posts_callback() {
		...
		if ( empty( $response['data'] ) ) {
			$response['error'] = esc_html__( 'Sorry, but nothing matched your search criteria.', 'bugster' );
		}
...
// Show pagination
if ( ! function_exists( 'bugster_show_pagination' ) ) {
	function bugster_show_pagination() {
		global $wp_query;
		// Pagination
		$pagination = bugster_get_theme_option( 'blog_pagination' );
		if ( 'pages' == $pagination ) {
			the_posts_pagination(
				array(
					'mid_size'           => 2,
					'prev_text'          => esc_html__( '<', 'bugster' ),
					'next_text'          => esc_html__( '>', 'bugster' ),
					'before_page_number' => '<span class="meta-nav screen-reader-text">' . esc_html__( 'Page', 'bugster' ) . ' </span>',
				)
			);
			...
			if ( $page_number < $wp_query->max_num_pages ) {
				...
				">
					<a class="nav-load-more" href="#" 
						...
						><span><?php esc_html_e( 'Load more posts', 'bugster' ); ?></span></a>
				</div>
				<?php
			}
			...
			<div class="nav-links-old">
				<span class="nav-prev"><?php previous_posts_link( is_search() ? esc_html__( 'Previous posts', 'bugster' ) : esc_html__( 'Newest posts', 'bugster' ) ); ?></span>
				<span class="nav-next"><?php next_posts_link( is_search() ? esc_html__( 'Next posts', 'bugster' ) : esc_html__( 'Older posts', 'bugster' ), $wp_query->max_num_pages ); ?></span>
			</div>
			<?php
		}
	}
...
if ( ! function_exists( 'bugster_single_comments_field' ) ) {
	function bugster_single_comments_field( $args ) {
