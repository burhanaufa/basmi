...
------------------------------------------------------------------------------- */
if ( ! function_exists( 'bugster_trx_addons_theme_specific_components' ) ) {
	add_filter( 'trx_addons_filter_components_editor', 'bugster_trx_addons_theme_specific_components' );
	function bugster_trx_addons_theme_specific_components( $enable = false ) {
		return BUGSTER_THEME_FREE
					? false     // Free version
					: false;     // Pro version or Developer mode
	}
}

if ( ! function_exists( 'bugster_trx_addons_theme_specific_setup1' ) ) {
	add_action( 'after_setup_theme', 'bugster_trx_addons_theme_specific_setup1', 1 );
	function bugster_trx_addons_theme_specific_setup1() {
		if ( bugster_exists_trx_addons() ) {
			add_filter( 'trx_addons_cv_enable', 'bugster_trx_addons_cv_enable' );
			add_filter( 'trx_addons_demo_enable', 'bugster_trx_addons_demo_enable' );
			add_filter( 'trx_addons_filter_edd_themes_market', 'bugster_trx_addons_edd_themes_market_enable' );
			add_filter( 'trx_addons_api_list', 'bugster_trx_addons_api_list' );
			add_filter( 'trx_addons_cpt_list', 'bugster_trx_addons_cpt_list' );
			add_filter( 'trx_addons_sc_list', 'bugster_trx_addons_sc_list' );
			add_filter( 'trx_addons_widgets_list', 'bugster_trx_addons_widgets_list' );
		}
	}
}

if ( ! function_exists( 'bugster_trx_addons_theme_specific_setup' ) ) {
	add_action( 'after_setup_theme', 'bugster_trx_addons_theme_specific_setup' );
	function bugster_trx_addons_theme_specific_setup() {
		if ( bugster_exists_trx_addons() ) {
			if ( ! is_admin() ) {
				add_action( 'bugster_action_before_post_meta', 'bugster_trx_addons_action_before_post_meta' );
			}
		}
	}
}
...
// CV
if ( ! function_exists( 'bugster_trx_addons_cv_enable' ) ) {
	//Handler of the add_filter( 'trx_addons_cv_enable', 'bugster_trx_addons_cv_enable');
	function bugster_trx_addons_cv_enable( $enable = false ) {
...
// Demo mode
if ( ! function_exists( 'bugster_trx_addons_demo_enable' ) ) {
	//Handler of the add_filter( 'trx_addons_demo_enable', 'bugster_trx_addons_demo_enable');
	function bugster_trx_addons_demo_enable( $enable = false ) {
...
// EDD Themes market
if ( ! function_exists( 'bugster_trx_addons_edd_themes_market_enable' ) ) {
	//Handler of the add_filter( 'trx_addons_filter_edd_themes_market', 'bugster_trx_addons_edd_themes_market_enable')) {
	function bugster_trx_addons_edd_themes_market_enable( $enable = false ) {
...
// API
if ( ! function_exists( 'bugster_trx_addons_api_list' ) ) {
	//Handler of the add_filter('trx_addons_api_list',	'bugster_trx_addons_api_list');
	function bugster_trx_addons_api_list( $list = array() ) {
...
// CPT
if ( ! function_exists( 'bugster_trx_addons_cpt_list' ) ) {
	//Handler of the add_filter('trx_addons_cpt_list',	'bugster_trx_addons_cpt_list');
	function bugster_trx_addons_cpt_list( $list = array() ) {
...
// Shortcodes
if ( ! function_exists( 'bugster_trx_addons_sc_list' ) ) {
	//Handler of the add_filter('trx_addons_sc_list',	'bugster_trx_addons_sc_list');
	function bugster_trx_addons_sc_list( $list = array() ) {
...
// Widgets
if ( ! function_exists( 'bugster_trx_addons_widgets_list' ) ) {
	//Handler of the add_filter('trx_addons_widgets_list',	'bugster_trx_addons_widgets_list');
	function bugster_trx_addons_widgets_list( $list = array() ) {
...
if ( ! function_exists( 'bugster_trx_addons_menu_cache' ) ) {
	add_filter( 'trx_addons_filter_menu_cache', 'bugster_trx_addons_menu_cache' );
	function bugster_trx_addons_menu_cache( $list = array() ) {
...
if ( ! function_exists( 'bugster_trx_addons_localize_script' ) ) {
	...
	function bugster_trx_addons_localize_script( $arr ) {
		$arr['alter_link_color'] = bugster_get_scheme_color( 'alter_link' );
		return $arr;
	}
}
...
if ( ! function_exists( 'bugster_trx_addons_sc_type' ) ) {
	add_filter( 'trx_addons_sc_type', 'bugster_trx_addons_sc_type', 10, 2 );
	function bugster_trx_addons_sc_type( $list, $sc ) {
		// To do: check shortcode slug and if correct - add new 'key' => 'title' to the list
		if ( 'trx_sc_blogger' == $sc ) {
			$list = bugster_array_merge( $list, bugster_get_list_blog_styles( false, 'sc' ) );
		}
...
if ( ! function_exists( 'bugster_trx_addons_sc_prepare_atts' ) ) {
	add_filter( 'trx_addons_filter_sc_prepare_atts', 'bugster_trx_addons_sc_prepare_atts', 10, 2 );
	function bugster_trx_addons_sc_prepare_atts( $atts, $sc ) {
		if ( 'trx_sc_blogger' == $sc ) {
			$list = bugster_get_list_blog_styles( false, 'sc' );
			if ( isset( $list[ $atts['type'] ] ) ) {
				$custom_type = '';
				if ( strpos( $atts['type'], 'blog-custom-' ) === 0 ) {
					$blog_id = bugster_get_custom_blog_id( $atts['type'] );
...
if ( ! function_exists( 'bugster_trx_addons_sc_atts' ) ) {
	add_filter( 'trx_addons_sc_atts', 'bugster_trx_addons_sc_atts', 10, 2 );
	function bugster_trx_addons_sc_atts( $atts, $sc ) {
...
if ( ! function_exists( 'bugster_trx_addons_sc_map' ) ) {
	add_filter( 'trx_addons_sc_map', 'bugster_trx_addons_sc_map', 10, 2 );
	function bugster_trx_addons_sc_map( $params, $sc ) {
		...
		if ( in_array(
			...
			$params['params'][] = array(
				'param_name'  => 'scheme',
				'heading'     => esc_html__( 'Color scheme', 'bugster' ),
				'description' => wp_kses_data( __( 'Select color scheme to decorate this block', 'bugster' ) ),
				'group'       => esc_html__( 'Colors', 'bugster' ),
				'admin_label' => true,
				'value'       => array_flip( bugster_get_list_schemes( true ) ),
				'type'        => 'dropdown',
			);
		}
		// Param 'color_style'
		$param = array(
			...
			'heading'          => esc_html__( 'Color style', 'bugster' ),
			'description'      => wp_kses_data( __( 'Select color style to decorate this block', 'bugster' ) ),
			'edit_field_class' => 'vc_col-sm-4',
			'admin_label'      => true,
			'value'            => array_flip( bugster_get_list_sc_color_styles() ),
			'type'             => 'dropdown',
		);
		...
		if ( 'trx_sc_blogger' == $sc ) {
			...
			$styles = bugster_storage_get( 'blog_styles' );
			unset( $styles['gallery'] );  // On gallery hover is 'icon' always
			$param      = array(
				'param_name'       => 'hover',
				'heading'          => esc_html__( 'Image hover', 'bugster' ),
				'description'      => wp_kses_data( __( 'Select hover effect for the featured image', 'bugster' ) ),
				'edit_field_class' => 'vc_col-sm-4',
				'value'            => array_flip( bugster_get_list_hovers( true ) ),
				'dependency'       => array(
					'element' => 'type',
...
if ( ! function_exists( 'bugster_trx_addons_sc_output' ) ) {
	add_filter( 'trx_addons_sc_output', 'bugster_trx_addons_sc_output', 10, 4 );
	function bugster_trx_addons_sc_output( $output, $sc, $atts, $content ) {

		if ( in_array( $sc, array( 'trx_sc_action' ) ) ) {
			if ( ! empty( $atts['scheme'] ) && ! bugster_is_inherit( $atts['scheme'] ) ) {
				$output = str_replace( 'class="sc_action ', 'class="sc_action scheme_' . esc_attr( $atts['scheme'] ) . ' ', $output );
			}
			if ( ! empty( $atts['color_style'] ) && ! bugster_is_inherit( $atts['color_style'] ) && 'default' != $atts['color_style'] ) {
...
if ( ! function_exists( 'bugster_trx_addons_sc_item_link_classes' ) ) {
	add_filter( 'trx_addons_filter_sc_item_link_classes', 'bugster_trx_addons_sc_item_link_classes', 10, 3 );
	function bugster_trx_addons_sc_item_link_classes( $class, $sc, $atts=array() ) {
		if ( 'sc_button' == $sc ) {
			if ( ! empty( $atts['color_style'] ) && ! bugster_is_inherit( $atts['color_style'] ) && 'default' != $atts['color_style'] ) {
...
if ( ! function_exists( 'bugster_trx_addons_sc_item_title_tag' ) ) {
	add_filter( 'trx_addons_filter_sc_item_title_tag', 'bugster_trx_addons_sc_item_title_tag' );
	function bugster_trx_addons_sc_item_title_tag( $tag = '' ) {
		return 'h1' == $tag ? 'h2' : $tag;
	}
}

// Return args for the item's button
if ( ! function_exists( 'bugster_trx_addons_sc_item_button_args' ) ) {
	add_filter( 'trx_addons_filter_sc_item_button_args', 'bugster_trx_addons_sc_item_button_args', 10, 3 );
	function bugster_trx_addons_sc_item_button_args( $args, $sc, $sc_args ) {
...
if ( ! function_exists( 'bugster_trx_addons_slider_title' ) ) {
	add_filter( 'trx_addons_filter_slider_title', 'bugster_trx_addons_slider_title', 10, 2 );
	function bugster_trx_addons_slider_title( $title, $data ) {
		$title = '';
		if ( ! empty( $data['title'] ) ) {
...
if ( ! function_exists( 'bugster_trx_addons_sc_googlemap_styles' ) ) {
	add_filter( 'trx_addons_filter_sc_googlemap_styles', 'bugster_trx_addons_sc_googlemap_styles' );
	function bugster_trx_addons_sc_googlemap_styles( $list ) {
		$list['dark'] = esc_html__( 'Dark', 'bugster' );
		return $list;
	}
}

// Show reactions in the single posts
if ( ! function_exists( 'bugster_trx_addons_action_before_post_meta' ) ) {
	//Handler of the add_action( 'bugster_action_before_post_meta', 'bugster_trx_addons_action_before_post_meta');
	function bugster_trx_addons_action_before_post_meta() {
		if ( bugster_exists_trx_addons() ) {
...
if ( ! function_exists( 'bugster_trx_addons_tiny_mce_style_formats' ) ) {
	add_filter( 'trx_addons_filter_tiny_mce_style_formats', 'bugster_trx_addons_tiny_mce_style_formats' );
	function bugster_trx_addons_tiny_mce_style_formats( $style_formats ) {
		...
		if ( false && is_array( $style_formats ) && count( $style_formats ) > 0 ) {
			foreach ( $style_formats as $k => $v ) {
				if ( esc_html__( 'List styles', 'bugster' ) == $v['title'] ) {
					$style_formats[ $k ]['items'][] = array(
						'title'    => esc_html__( 'Arrow', 'bugster' ),
						'selector' => 'ul',
...
if ( ! function_exists( 'bugster_trx_addons_allow_override_header_image' ) ) {
	add_filter( 'bugster_filter_allow_override_header_image', 'bugster_trx_addons_allow_override_header_image' );
	function bugster_trx_addons_allow_override_header_image( $allow ) {
		return is_single()
				&& (
					bugster_is_team_page()
					|| bugster_is_cars_page()
					|| bugster_is_cars_agents_page()
					|| bugster_is_properties_agents_page()
					)
				? false
				: $allow;
	}
...
if ( ! function_exists( 'bugster_trx_addons_thumb_size' ) ) {
	add_filter( 'trx_addons_filter_thumb_size', 'bugster_trx_addons_thumb_size', 10, 2 );
	function bugster_trx_addons_thumb_size( $thumb_size = '', $type = '' ) {
			 if ((false) && ($type == 'team-default')) {
				$thumb_size = bugster_get_thumb_size('big');
			 }
		return $thumb_size;
	}
}
...
if ( ! function_exists( 'bugster_trx_addons_override_options_fields' ) ) {
	add_filter( 'trx_addons_filter_meta_box_fields', 'bugster_trx_addons_override_options_fields', 10, 2 );
	function bugster_trx_addons_override_options_fields( $mb, $post_type ) {
		if ( defined( 'TRX_ADDONS_CPT_TEAM_PT' ) && TRX_ADDONS_CPT_TEAM_PT == $post_type ) {
			if ( ! isset( $mb['email'] ) ) {
				$mb['email'] = array(
					'title'   => esc_html__( 'E-mail', 'bugster' ),
					'desc'    => wp_kses_data( __( "Team member's email", 'bugster' ) ),
					'std'     => '',
					'details' => true,
					'type'    => 'text',
				);
...
// Hide yandex api && instagram api
if( !function_exists('name_of_theme_remove_parent_filters') ) {
	add_filter( 'trx_addons_filter_options', 'name_of_theme_remove_parent_filters', 10, 1 );
	function name_of_theme_remove_parent_filters($list){ 
		unset($list['api_yandex_info']);
		$list['api_yandex']['type'] = 'hidden';
		$list['api_yandex_load']['type'] = 'hidden';
		$list['api_yandex_marker']['type'] = 'hidden';
		$list['api_yandex_cluster']['type'] = 'hidden';
		$list['api_instagram_info']['type'] = 'hidden';
		$list['api_instagram_client_id']['type'] = 'hidden';
		$list['api_instagram_client_secret']['type'] = 'hidden';
		$list['api_instagram_get_access_token']['type'] = 'hidden';
		$list['api_instagram_access_token']['type'] = 'hidden';
		$list['emotions_info']['type'] = 'hidden';
		$list['emotions_allowed']['type'] = 'hidden';
		$list['emotions']['type'] = 'hidden';
		return $list;
	}
}
