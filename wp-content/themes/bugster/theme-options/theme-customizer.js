/* global bugster_color_schemes, bugster_dependencies, Color */
...
( function( api ) {
	...
	for (var i in bugster_color_schemes) {
		cssTemplate['scheme_' + i] = wp.template( 'bugster-color-scheme-' + i );
	}
	// Add Template with theme fonts
	cssTemplate['theme_fonts'] = wp.template( 'bugster-fonts' );
	// Add Template with theme vars
	cssTemplate['theme_vars'] = wp.template( 'bugster-vars' );
	...
	api.bind(
		'ready', function() {
			...
			// Add 'reset' button
			jQuery( '#customize-header-actions #save' ).before( '<input type="button" class="button customize-action-reset" value="' + bugster_customizer_vars['msg_reset'] + '">' );
			jQuery( '#customize-header-actions .customize-action-reset' ).on(
				'click', function(e) {
					if (confirm( bugster_customizer_vars['msg_reset_confirm'] )) {
			...
			if (bugster_customizer_vars && bugster_customizer_vars['theme_name_suffix']) {
				jQuery( '#customize-info .site-title' ).append( bugster_customizer_vars['theme_name_suffix'] );
			}

			// Blur the "load fonts" fields - regenerate options lists in the font-family controls
			jQuery( '#customize-theme-controls [id^="customize-control-load_fonts"]' ).on( 'focusout', bugster_customizer_update_load_fonts );

			// Click on the actions button
			jQuery( '#customize-theme-controls .control-section .customize-control-button input[type="button"]' ).on( 'click', bugster_customizer_actions );

			// Check dependencies in the each section on ready
			jQuery( '#customize-theme-controls .control-section' ).each(
				function () {
					bugster_customizer_check_dependencies( jQuery( this ) );
				}
			);
			...
			jQuery( '#customize-theme-controls .control-section > .accordion-section-title' ).on(
				'click', function() {
					...
					if (id !== '') {
						...
						if (section.length > 0) {
							bugster_customizer_check_dependencies( section );
						}
					}
				}
			);
	...
	api.bind(
		'change', function(obj) {
			...
			bugster_customizer_check_dependencies( jQuery( '#customize-theme-controls #customize-control-' + obj.id ).parents( '.control-section' ) );
			bugster_customizer_refresh_preview( obj );
		}
	);
	...
	api.bind(
		'refresh_schemes', function() {
			...
			for (var i in cssTemplate) {
				...
				i = i.replace( 'scheme_', '' );
				if (typeof bugster_color_schemes[i] === 'undefined') {
					delete cssTemplate['scheme_' + i];
					jQuery( '#tmpl-bugster-color-scheme-' + i ).remove();
				} else if (tpl_idx == '') {
					tpl_idx     = i;
					tpl_content = jQuery( '#tmpl-bugster-color-scheme-' + i ).html().trim();
				}
			}
			// Add new templates
			...
			for (var i in bugster_color_schemes) {
				if (typeof cssTemplate['scheme_' + i] === 'undefined') {
					jQuery( '#tmpl-bugster-color-scheme-' + tpl_idx )
					.clone()
					.attr( 'id', 'tmpl-bugster-color-scheme-' + i )
					.html( tpl_content.replace( regex, '.scheme_' + i ) )
					.insertAfter( jQuery( '#tmpl-bugster-color-scheme-' + tpl_idx ) );
					cssTemplate['scheme_' + i] = wp.template( 'bugster-color-scheme-' + i );
				}
			}
		}
	...
	for (var action in bugster_customizer_vars['actions']['expand']) {
		if (action == 'length') {
			continue;
		}
		if (bugster_customizer_vars['actions']['expand'][action]['type'] == 'panel') {
			api.panel(
				action, function( panel ) {
					panel.expanded.bind(
						function( isExpanded ) {
							if ( isExpanded ) {
								var data = bugster_customizer_vars['actions']['expand'][panel.id];
			...
			api.section(
				action, function( section ) {
					section.expanded.bind(
						function( isExpanded ) {
							if ( isExpanded ) {
								var data = bugster_customizer_vars['actions']['expand'][section.id];
								if (typeof data['url'] !== 'undefined' && data['url'] !== '') {
									api.previewer.previewUrl.set( data['url'] );
								}
								if (typeof data['callback'] !== 'undefined' && data['callback'] !== '' && typeof window[data['callback']] === 'function') {
	...
	function bugster_customizer_get_field_value(fld) {
	...
	function bugster_customizer_check_dependencies(cont) {
		cont.find( '.customize-control' ).each(
			function() {
				...
				var fld    = null, val = '', i;
				var depend = false;
				for (fld in bugster_dependencies) {
					if (fld == id) {
				...
				if (depend) {
					...
					for (i in depend) {
						...
						if (fld.length > 0) {
							val = bugster_customizer_get_field_value( fld );
					...
					if (((dep_cnt > 0 || dep_all == 0) && dep_cmp == 'or') || (dep_cnt == dep_all && dep_cmp == 'and')) {
						ctrl.show().removeClass( 'bugster_options_no_use' );
					} else {
						ctrl.hide().addClass( 'bugster_options_no_use' );
					}
				}
				...
				if (false && id == 'color_scheme') {
					...
					if (fld.length > 0) {
						val     = bugster_customizer_get_field_value( fld );
						var num = 0;
						for (i in bugster_color_schemes) {
							num++;
							if (i == val) {
								break;
							}
	...
	function bugster_customizer_refresh_preview(obj) {
		...
		if (updateCSS) {
			...
			if (id == 'scheme_storage') {
				...
				// Any color in the scheme_storage is changed
			} else if (Object.values( bugster_sorted_schemes ).indexOf( id ) >= 0) {
				forceUpdateCSS = true;
				...
				if (var_name !== undefined) {
					...
					bugster_customizer_update_theme_vars( jQuery( '[data-customize-setting-link="' + id + '"]' ).data( 'var_name' ), val );
					processed = true;
				}

				// Any theme fonts parameter is changed
			} else {
				for (opt in bugster_theme_fonts) {
					for (rule in bugster_theme_fonts[opt]) {
						if (opt + '_' + rule == id) {
							// Store new value to the fonts table
							bugster_customizer_update_theme_fonts( opt, rule, val );
							processed = true;
							break;
						}
					}
	...
	// Actions buttons
	function bugster_customizer_actions(e) {
	...
	function bugster_customizer_update_theme_vars(opt, value) {
		bugster_theme_vars[opt] = parseFloat( value );
	}

	// Store new value in the theme fonts
	function bugster_customizer_update_theme_fonts(opt, rule, value) {
		bugster_theme_fonts[opt][rule] = value;
	}

	// Change theme fonts options if load fonts is changed
	function bugster_customizer_update_load_fonts() {
		var opt_list = [], i, tag, sel, opt, name = '', family = '', val = '', new_val = '', sel_idx = 0;
		updateCSS    = false;
		for (i = 1; i <= bugster_customizer_vars['max_load_fonts']; i++) {
	...
	function bugster_customizer_update_css() {
		...
		var vars = bugster_customizer_add_theme_vars( bugster_theme_vars );

		// Make styles and add its to the css
		css += bugster_customizer_prepare_html_value( cssTemplate['theme_vars']( vars ) );

		// Add theme specific fonts rules
		var fonts = bugster_customizer_add_theme_fonts( bugster_theme_fonts );

		// Make styles and add its to the css
		css += bugster_customizer_prepare_html_value( cssTemplate['theme_fonts']( fonts ) );

		// Add colors
		var sorted = {}, scheme = '';
		for (var param in bugster_sorted_schemes) {
			if (param == 'length') {
				continue;
			}
			scheme = api( bugster_sorted_schemes[param] )();
			if (scheme && scheme !== 'inherit') {
				sorted[scheme] = 1;
			}
		}
		for (scheme in bugster_color_schemes) {
			sorted[scheme] = 1;
		}
		for (scheme in sorted) {
			var i, colors = [];
			// Copy all colors to the temp var
			for (i in bugster_color_schemes[scheme].colors) {
				colors[i] = bugster_color_schemes[scheme].colors[i];
			}
			// Add theme specific colors and tints
			colors = bugster_customizer_add_theme_colors( colors );
			...
			var tmpl = jQuery( '#tmpl-bugster-color-scheme-' + scheme ).html().trim();
			for (i in colors) {
	...
	function bugster_customizer_add_theme_colors(colors) {
		if (bugster_additional_colors) {
			var clr = '', v = '';
			for (var k in bugster_additional_colors) {
				v   = bugster_additional_colors[k];
				clr = colors[v['color']];
	...
	function bugster_customizer_add_theme_fonts(fonts) {
		var rez = [];
		for (var tag in fonts) {
			...
			rez[tag + '_font-size']       = typeof fonts[tag]['font-size'] != 'undefined'
									&& fonts[tag]['font-size'] != 'inherit'
												? 'font-size:' + bugster_customizer_prepare_css_value( fonts[tag]['font-size'] ) + ";"
												: '';
			...
			rez[tag + '_margin-top']      = typeof fonts[tag]['margin-top'] != 'undefined'
									...
									&& fonts[tag]['margin-top'] != 'inherit'
												? 'margin-top:' + bugster_customizer_prepare_css_value( fonts[tag]['margin-top'] ) + ";"
												: '';
			rez[tag + '_margin-bottom']   = typeof fonts[tag]['margin-bottom'] != 'undefined'
									&& fonts[tag]['margin-bottom'] !== ''
									&& fonts[tag]['margin-bottom'] != 'inherit'
												? 'margin-bottom:' + bugster_customizer_prepare_css_value( fonts[tag]['margin-bottom'] ) + ";"
												: '';
		}
		return rez;
	}
	...
	function bugster_customizer_add_theme_vars(vars) {
		var rez = [];
		for (var i in vars) {
			if (i == 'rad') {
				if (vars['rad'] == '') {
				...
				rez['rad']   = bugster_customizer_prepare_css_value( vars['rad'] );
				...
				rez['page']              = bugster_customizer_prepare_css_value( vars['page'] );
				rez['page_boxed']        = bugster_customizer_prepare_css_value( vars['page'] + 2 * vars['page_boxed_extra'] );
				rez['sidebar']           = bugster_customizer_prepare_css_value( vars['sidebar'] );
				rez['gap']               = bugster_customizer_prepare_css_value( vars['gap'] );
				rez['content']           = bugster_customizer_prepare_css_value( vars['page'] - vars['sidebar'] - vars['gap'] );
				rez['sidebar_gap']       = bugster_customizer_prepare_css_value( vars['sidebar'] + vars['gap'] );
				rez['sidebar_prc']       = vars['sidebar'] / vars['page'];
				rez['gap_prc']           = vars['gap'] / vars['page'];
				rez['page_1_1']          = bugster_customizer_prepare_css_value( vars['page'] );
				rez['page_1_2']          = bugster_customizer_prepare_css_value( vars['page'] / 2 );
				rez['page_1_3']          = bugster_customizer_prepare_css_value( vars['page'] / 3 );
				rez['page_2_3']          = bugster_customizer_prepare_css_value( vars['page'] / 3 * 2 );
				rez['page_1_4']          = bugster_customizer_prepare_css_value( vars['page'] / 4 );
				rez['page_3_4']          = bugster_customizer_prepare_css_value( vars['page'] / 4 * 3 );
				var margins              = [10,20,30,40,60];
				for (var m = 0; m < margins.length; m++) {
					rez['page' + margins[m]]    = bugster_customizer_prepare_css_value( vars['page'] + margins[m] );
					rez['content' + margins[m]] = bugster_customizer_prepare_css_value( vars['page'] - vars['gap'] - vars['sidebar'] + margins[m] );
				}
			}
		}
		return rez;
	...
	// Add ed to css value
	function bugster_customizer_prepare_css_value(val) {
		if (val !== '' && val != 'inherit') {
			var ed = ('' + val).substr( -1 );
			if ('0' <= ed && ed <= '9') {
				val += 'px';
	...
	function bugster_customizer_prepare_html_value(val) {
