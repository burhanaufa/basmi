...
( function( $ ) {
	...
	var $style = $( '#bugster-customizer-inline-css' ),
		api    = wp.customize;

	// Prepare inline styles in the preview window
	if ( $style.length == 0 ) {
		var style_container = '<' + 'style type="text/css" id="bugster-customizer-inline-css" />',
			$theme_css      = $( '#bugster-main-inline-css' );
		if ($theme_css.length == 0) {
			$theme_css = $( '#bugster-main-css' );
		}
		if ($theme_css.length == 0) {
		...
		$style = $( '#bugster-customizer-inline-css' );
	}
			...
			api.preview.bind(
				'refresh-other-controls', function( obj ) {
					...
					if (id.indexOf( 'body_style' ) == 0) {
						...
						$( '.sc_layouts_logo .logo_text' ).html( bugster_prepare_macros( val ) );

					} else if (id == 'blogdescription') {
						$( '.sc_layouts_logo .logo_slogan' ).html( bugster_prepare_macros( val ) );

					} else if (id == 'copyright') {
						$( '.copyright_text' ).html( bugster_prepare_macros( val ) );
						...
						if (id == 'front_page_bg_image') {
							...
							if (color_type == 'none') {
								...
								color = typeof bugster_color_schemes[scheme] != 'undefined' ? bugster_color_schemes[scheme]['colors']['bg_color'] : '';
							}
							$( '.front_page_section_' + section + '_inner' ).css( 'background-color', color == '' ? 'transparent' : ( mask == 1 ? color : bugster_hex2rgba( color, mask ) ) );
							...
							.height( 'auto' )
							.toggleClass( 'bugster-full-height sc_layouts_flex sc_layouts_columns_middle', val );
							$( window ).trigger( 'resize' );
						}
					}
				}
