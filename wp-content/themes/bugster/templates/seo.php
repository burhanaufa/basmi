...
if ( bugster_is_on( bugster_get_theme_option( 'seo_snippets' ) ) ) {
	...
	<div class="structured_data_snippets">
		...
		<div itemscope itemprop="publisher" itemtype="https://schema.org/Organization">
			...
			<?php
			$bugster_logo_image = bugster_get_logo_image();
			if ( ! empty( $bugster_logo_image['logo'] ) ) {
				?>
				<meta itemprop="logo" itemtype="https://schema.org/ImageObject" content="<?php echo esc_url( $bugster_logo_image['logo'] ); ?>">
				<?php
			}
			?>
		</div>
		<?php
		if ( bugster_get_theme_option( 'show_author_info' ) != 1 || ! is_single() || is_attachment() || ! get_the_author_meta( 'description' ) ) {
			?>
